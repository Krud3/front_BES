...
const PRSheet: React.FC<PRSheetProps> = ({ setNodes, setLinks }) => {
    ...
    if (selectedSimulation) {
      try {
        const response = await fetch(
          `http://localhost:5000/csv/${selectedSimulation}`,
        );
        if (!response.ok) {
          throw new Error("Failed to fetch CSV file.");
        }
        const csvText = await response.text();
        const csvFile = new File([csvText], selectedSimulation, {
          type: "text/csv",
        });
  ...
  const handleViewData = () => {
    if (selectedSimulation) {
      navigate("/board/table-data", {
        state: { simulation: selectedSimulation },
      });
    } else {
      alert("Please select a simulation before proceeding.");
    }
            ...
            <div className="py-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Select a previous simulation to load, then click on View
                    Data or View Graph.
                  </CardDescription>
                </CardHeader>
                <CardContent>
