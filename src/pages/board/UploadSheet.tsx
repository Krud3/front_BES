...
const UploadSheet: React.FC<UploadSheetProps> = ({ setNodes, setLinks }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "RESULTS" | "NETWORK" | "AGENT" | "UPLOAD SIMULATION"
  >("UPLOAD SIMULATION");
            ...
            if (!nodesMap.has(agentId)) {
              nodesMap.set(agentId, {
                ...
                publicBeliefsOverTime: [
                  { date: roundDate, value: publicBelief },
                ],
  ...
  const handleProceed = async () => {
    if (selectedFile) {
      try {
        ...
        alert(
          "Hubo un error al procesar el archivo CSV. Por favor, intenta de nuevo.",
        );
      }
    } else {
  ...
  return (
    <Sheet>
      ...
      <SheetContent side="left" className="w-120">
        {" "}
