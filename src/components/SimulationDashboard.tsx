...
export const SimulationDashboard: React.FC = () => {
  const {
    ...
    connect,
    disconnect,
    latestSimulationData: simulationData,
    clearData,
  ...
  const metrics = useMemo(() => {
    ...
    const averageBelief =
      beliefs.reduce((sum, b) => sum + b, 0) / totalAgents || 0;
  ...
  const historicalMetrics = useMemo(() => {
    return history.map((dataPoint) => {
      ...
      const variance =
        beliefs.reduce((sum, b) => sum + Math.pow(b - avg, 2), 0) /
          beliefs.length || 0;
  ...
  const handlePreviousRound = () => {
    ...
    const currentIndex = history.findIndex(
      (h) => h.round === (selectedRound || metrics.round),
    );
    if (currentIndex > 0) {
