...
export function CustomSimulationForm() {
  ...
  const writeUint32 = (buffer: ArrayBuffer, offset: number, value: number): void => { new DataView(buffer).setUint32(offset, value, true); };
  ...
  const calculateBufferSize = (): number => {
    ...
    agents.forEach(agent => { size += 1 + stringToBytes(agent.name).length; });
    size = Math.ceil(size / 4) * 4;
  ...
  const buildBuffer = (): ArrayBuffer => {
    ...
    uint8View[offset] = networkNameBytes.length; offset += 1;
    ...
    agents.forEach((agent, i) => writeFloat32(buffer, offset + i * 4, agent.toleranceRadius));
    offset += agents.length * 4;
    agents.forEach((agent, i) => writeFloat32(buffer, offset + i * 4, agent.toleranceOffset));
    offset += agents.length * 4;
    agents.forEach((agent, i) => { uint8View[offset + i] = agent.silenceStrategy; });
    offset += agents.length;
    agents.forEach((agent, i) => { uint8View[offset + i] = agent.silenceEffect; });
    offset += agents.length;

    agents.forEach(agent => {
      const nameBytes = stringToBytes(agent.name);
      uint8View[offset] = nameBytes.length; offset += 1;
    ...
    neighbors.forEach((neighbor, i) => { uint8View[offset + i] = neighbor.bias; });
    offset += neighbors.length;

    neighbors.forEach(neighbor => {
      const sourceBytes = stringToBytes(neighbor.source);
      uint8View[offset] = sourceBytes.length; offset += 1;
      uint8View.set(sourceBytes, offset); offset += sourceBytes.length;
    });
    neighbors.forEach(neighbor => {
  ...
  const handleAddAgent = useCallback(() => {
    const newAgent: CustomAgent = {
      id: crypto.randomUUID(), name: `Agent ${agents.length + 1}`, initialBelief: 0.5, toleranceRadius: 0.3,
      toleranceOffset: 0.1, silenceStrategy: 0, silenceEffect: 0, thresholdValue: 0.5, confidenceValue: 0.5, updateValue: 1
    };
  ...
  const handleRemoveAgent = useCallback((agentId: string) => {
    ...
    setCustomForm(prev => ({
        ...
        neighbors: prev.neighbors.filter(n => n.source !== agentToRemove.name && n.target !== agentToRemove.name)
    }));
  }, [agents, setCustomForm]);

  const handleUpdateAgent = useCallback((agentId: string, updates: Partial<CustomAgent>) => {
    setCustomForm(prev => {
        const oldAgent = prev.agents.find(a => a.id === agentId);
        const newAgents = prev.agents.map(agent => agent.id === agentId ? { ...agent, ...updates } : agent);
        let newNeighbors = prev.neighbors;
        if (updates.name && oldAgent && updates.name !== oldAgent.name) {
            newNeighbors = prev.neighbors.map(n => ({ ...n,
  ...
  const handleAddNeighbor = useCallback(() => {
    ...
    neighbors.forEach(n => { sourceCounts[n.source] = (sourceCounts[n.source] || 0) + 1; });
    ...
    for (const sourceAgent of agents) {
      ...
      for (const targetAgent of agents) {
        if (sourceAgent.id !== targetAgent.id) {
          if (!usedPairs.has(`${sourceAgent.name}-${targetAgent.name}`)) { source = sourceAgent.name; target = targetAgent.name; break; }
        }
      }
      if (source) break;
    }
    if (!source || !target) return;
    const newNeighbor: Neighbor = { id: crypto.randomUUID(), source, target, influence: 0.5, bias: 0 };
    setCustomForm(prev => ({...prev, neighbors: [...prev.neighbors, newNeighbor]}));
  }, [agents, neighbors, setCustomForm]);

  const handleRemoveNeighbor = useCallback((neighborId: string) => {
    setCustomForm(prev => ({ ...prev, neighbors: prev.neighbors.filter(n => n.id !== neighborId) }));
  }, [setCustomForm]);

  const handleUpdateNeighbor = useCallback((neighborId: string, updates: Partial<Neighbor>) => {
    setCustomForm(prev => ({ ...prev, neighbors: prev.neighbors.map(n => n.id === neighborId ? { ...n, ...updates } : n) }));
  }, [setCustomForm]);
  ...
  const getAvailableSources = useCallback((currentNeighborId: string) => {
    ...
    const sourceCounts: Record<string, number> = {};
    neighbors.forEach(n => { if (n.id !== currentNeighborId) { sourceCounts[n.source] = (sourceCounts[n.source] || 0) + 1; } });
    const maxSourceCount = agents.length - 1;
    return agents.map(a => a.name).filter(name => {
  ...
  const getAvailableTargets = useCallback((source: string, currentNeighborId: string) => {
    const usedPairs = neighbors.filter(n => n.id !== currentNeighborId).map(n => `${n.source}-${n.target}`);
    return agents.map(a => a.name).filter(name => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8080/custom', { method: 'POST', headers: { 'Content-Type': 'application/octet-stream' }, body: buffer });
    ...
    <TooltipProvider>
      <Card className="w-full max-w-4xl mx-auto border-none shadow-none">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="stopThreshold" type="number" min={0} step={0.0001} value={stopThreshold} onChange={(e) => setCustomForm(p => ({...p, stopThreshold: parseFloat(e.target.value) || 0}))} className="h-9"/>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={saveMode.toString()} onValueChange={(v) => setCustomForm(p => ({...p, saveMode: parseInt(v)}))}>
                    <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                    <SelectContent>{Object.entries(SAVE_MODES).map(([value, label]) => (<SelectItem key={value} value={value}>{label}</SelectItem>))}</SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
                <Label htmlFor="networkName" className="text-sm font-medium">Network Name</Label>
                <Input id="networkName" value={networkName} onChange={(e) => setCustomForm(p => ({...p, networkName: e.target.value}))} maxLength={255} className="h-9"/>
              </div>
            </div>
          </div>
          <Separator />
            ...
            <div className="space-y-4">
              {agents.map((agent) => (
                <Card key={agent.id} className="relative overflow-hidden p-4 pr-14">
                    {agents.length > 1 && (<Button variant="destructive" size="icon" onClick={() => handleRemoveAgent(agent.id)} className="absolute top-3 right-3 h-8 w-8 z-10"><Trash2 className="w-4 h-4" /></Button>)}
                    <div className="grid grid-cols-3 gap-3 mb-3">
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">Agent Name</Label><Input value={agent.name} onChange={(e) => handleUpdateAgent(agent.id, {name: e.target.value})} placeholder="Agent name" className="h-9"/></div>
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">Silence Strategy</Label><Select value={agent.silenceStrategy.toString()} onValueChange={(v) => handleUpdateAgent(agent.id, {silenceStrategy: parseInt(v)})}><SelectTrigger className="h-9"><SelectValue/></SelectTrigger><SelectContent>{Object.entries(SILENCE_STRATEGIES).map(([value, label]) => (<SelectItem key={value} value={value}>{label}</SelectItem>))}</SelectContent></Select></div>
                        <div className="space-y-1"><Label className="text-xs text-muted-foreground">Silence Effect</Label><Select value={agent.silenceEffect.toString()} onValueChange={(v) => handleUpdateAgent(agent.id, {silenceEffect: parseInt(v)})}><SelectTrigger className="h-9"><SelectValue/></SelectTrigger><SelectContent>{Object.entries(SILENCE_EFFECTS).map(([value, label]) => (<SelectItem key={value} value={value}>{label}</SelectItem>))}</SelectContent></Select></div>
                    </div>
                    <Separator className="my-3"/>
                    {(() => {
                        const needsThreshold = agent.silenceStrategy === 2;
                        const needsConfidence = agent.silenceStrategy === 3;
                        const gridClass = needsConfidence ? "grid gap-4 grid-cols-5" : needsThreshold ? "grid gap-4 grid-cols-4" : "grid gap-4 grid-cols-3";
                        return (
                  ...
                  <Card key={neighbor.id} className="overflow-hidden p-3">
                    <div className="flex justify-between items-center gap-3 mb-3">
                        <div className="flex gap-2 items-center flex-1">
                          <Select value={neighbor.source} onValueChange={(v) => handleUpdateNeighbor(neighbor.id, {source: v})}><SelectTrigger className="h-9 w-[130px]"><SelectValue placeholder="Source"/></SelectTrigger><SelectContent>{getAvailableSources(neighbor.id).map(name => (<SelectItem key={name} value={name}>{name}</SelectItem>))}</SelectContent></Select>
                          <span className="text-muted-foreground">→</span>
                          <Select value={neighbor.target} onValueChange={(v) => handleUpdateNeighbor(neighbor.id, {target: v})}><SelectTrigger className="h-9 w-[130px]"><SelectValue placeholder="Target" /></SelectTrigger><SelectContent>{getAvailableTargets(neighbor.source, neighbor.id).map(name => (<SelectItem key={name} value={name}>{name}</SelectItem>))}</SelectContent></Select>
                        </div>
                        <Button variant="ghost" size="icon" onClick={() => handleRemoveNeighbor(neighbor.id)} className="h-8 w-8 hover:bg-destructive/10 hover:text-destructive"><Trash2 className="w-4 h-4" /></Button>
                    </div>
                    <Separator className="mb-3" />
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <div className="flex items-center justify-between"><Label className="text-xs text-muted-foreground">Influence</Label><span className="text-xs font-medium">{neighbor.influence.toFixed(2)}</span></div>
                            <Slider min={0} max={1} step={0.01} value={[neighbor.influence]} onValueChange={(v) => handleUpdateNeighbor(neighbor.id, { influence: v[0] })} className="mt-2"/>
                        </div>
                        <div className="space-y-1">
                            <Label className="text-xs text-muted-foreground">Bias Type</Label>
                            <Select value={neighbor.bias.toString()} onValueChange={(v) => handleUpdateNeighbor(neighbor.id, { bias: parseInt(v) })}>
                                <SelectTrigger className="h-9"><SelectValue /></SelectTrigger>
                                <SelectContent>{Object.entries(BIASES).map(([value, label]) => (<SelectItem key={value} value={value}>{label}</SelectItem>))}</SelectContent>
                            </Select>
                        </div>
                    </div>
                  </Card>
          ...
          {submitSuccess && (<Alert className="border-green-500"><Check className="h-4 w-4 text-green-600" /><AlertDescription>Simulation started successfully!</AlertDescription></Alert>)}
          ...
          <div className="flex justify-end items-center gap-4 pt-6 border-t">
            {!isValid && (<p className="text-sm text-muted-foreground">Please ensure all fields are valid</p>)}
            <Button onClick={handleSubmit} disabled={!isValid || isSubmitting} size="lg" className="min-w-[150px]">
              {isSubmitting ? (<><div className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-background border-t-transparent" />Submitting...</>) : ("Run Simulation")}
            </Button>
          </div>
        </CardContent>
      </Card>
